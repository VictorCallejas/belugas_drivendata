...
class Matchformer_SEA_lite(nn.Module):
    def __init__(self, img_size=224, in_chans=3, embed_dims=[128, 192, 256, 512], num_heads=[1, 2, 4, 8],sr_ratios=[8,4,2,1]
...
class Matchformer(nn.Module):
    ...
    def __init__(self):
        ...
        self.backbone = Matchformer_SEA_lite()
        #self.coarse_matching = CoarseMatching(config['match_coarse'])
        #self.fine_preprocess = FinePreprocess(config)
        #self.fine_matching = FineMatching()
        
        self.mlp_head = nn.Linear(100, 1)
    
    
    def forward(self, query, reference):
        
        x = torch.cat([query, reference], dim=0)
        feats_q, feats_r = self.backbone(x)
        print(feats_r.shape,feats_q.shape)
